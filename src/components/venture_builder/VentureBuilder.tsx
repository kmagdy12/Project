...
import LegalReadinessInputs from './business_setup/legal_readiness/LegalReadinessInputs';
...
const VentureBuilder: React.FC<VentureBuilderProps> = ({ onBack }) => {
  ...
  // Legal Readiness Data
  const [legalReadinessData, setLegalReadinessData] = useState<any>({
    incorporation: {
      ...
      case 'setup':
        return (
          <VentureBuildingLayout
            ...
            completionPercentage={completionPercentage}
            leftColumn={<AIChatHistory />}
            middleColumn={<div>Business Setup Chat</div>}
            rightColumn={
              <div className="p-4 h-full overflow-y-auto">
                <LegalReadinessInputs
                  legalReadinessData={legalReadinessData}
                  onUpdateLegalReadiness={setLegalReadinessData}
                  onContinue={() => handleNextStage('investment_readiness')}
                  onBack={() => handleNextStage('product_creation')}
                  onSkip={() => handleNextStage('investment_readiness')}
                  onSaveDraft={() => console.log('Saving draft...')}
                />
              </div>
            }
