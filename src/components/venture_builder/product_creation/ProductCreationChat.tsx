...
const ProductCreationChat: React.FC<ProductCreationChatProps> = ({ onSendMessage, currentSubStage }) => {
  ...
  const subStages = [
    { id: 'ux', label: 'UX', icon: Layers, description: 'User experience design' }, // Keep this for UX
    // Add Prototype and MVP stages
  ...
  const getAgentName = () => {
    switch (currentSubStage) {
      ...
      case 'prototype':
        return 'Prototype Builder Agent';
      case 'prototype':
  ...
  const getAgentFunction = () => {
    switch (currentSubStage) {
      ...
      case 'prototype':
        return 'Pre-fills prototype specs, optimizes development approach, explains prototyping methods';
      case 'prototype':
